...
class TestIndexEndpoint(unittest.TestCase):
    ...
    def test_index(self):
        ...
        self.assertEqual(str(json.loads(response.get_data())['message']), 'Welcome to The API!')

    def test_hello(self):
        """Assert that our hello route returns the correct messsage"""
        response = self.app.get('/hello/arnold')
        self.assertEqual(str(json.loads(response.get_data())['message']), 'Hello, arnold. Welcome to The API')
